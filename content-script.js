var element;
element = document.querySelector('.apphub_HeaderStandardTop');

var div = document.createElement('a');

div.setAttribute('class', 'epic-games-free-status');
div.style.margin = '3px 0px 8px 42px';
...
div.style.backgroundColor = '#2F2D2E';

element.appendChild(div);

const container = document.querySelector('.epic-games-free-status');

const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
svg.setAttribute('class', 'epic-games-free-status-svg');

container.appendChild(svg);

var svg2 = document.querySelector('.epic-games-free-status-svg'); //Get svg element
...
svg2.setAttribute('width', '34px');
svg2.setAttribute('height', '36px');
svg2.setAttribute('viewBox', '0 0 1024 1024');
svg2.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
svg2.appendChild(newElement);

var gameName = document.querySelector('.apphub_AppName');
...
for (let index = 0; index < GameListFree.length; index++) {
  console.log(index + '-' + GameListFree.length);

  if (GameListFree[index].name == gameName.innerHTML) {
    const textNode = document.createTextNode(
      'This game was given for free by Epic Games | ' + GameListFree[index].date
    );
    console.log('geldi1');
    div.appendChild(textNode);
    break;
  }
  if (index == GameListFree.length - 1) {
    console.log('geldi2');
    div.style.display = '';
